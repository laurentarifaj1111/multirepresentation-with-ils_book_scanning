...
from greedy_with_calculated_initial.solution.Operators import ReorderLibsTweakOperator, ShuffleLibsTweakOperator, \
    ReplaceLibsTweakOperator
...
from ils.models import Parser, Solver
...
def main():
    ...
    for file_name in file_names:
        ...
        print(
            f"Solution for instance: {file_name}, score: {solution_v1.fitness_score}, from solution with representations v1")
        fitness = sol.fitness_score
        results.append(
            Results(file_name, "variable_neighborhood_search", initial_score, other_representation_score, fitness))

        print(f"Final Fitness for instance {file_name} is : {fitness}")

        sol.export("./output/v1/final-solution" + file_name)
        SolutionOutputWriter.write_solution_to_file(improved_solution, file_name)

        Utils.write_results(results, "v9")


def hill_climbing(current_solution: SolutionRepresentation, instance_value: Instance,
